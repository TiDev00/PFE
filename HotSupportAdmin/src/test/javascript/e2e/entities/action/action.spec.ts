...
describe('Action e2e test', () => {
  ...
  it('should create and save Actions', async () => {
    ...
    await promise.all([
      ...
      actionUpdatePage.setDescActionInput('descAction'),
      actionUpdatePage.processSelectLastOption(),
    ]);

    expect(await actionUpdatePage.getActionNameInput()).to.eq('actionName', 'Expected ActionName value to be equals to actionName');
    expect(await actionUpdatePage.getDescActionInput()).to.eq('descAction', 'Expected DescAction value to be equals to descAction');
