...
/**
 * REST controller for managing {@link sn.sonatel.dsi.eai.domain.Action}.
 */
@RestController
@RequestMapping("/api")
public class ActionResource {
     ...
     */
    @GetMapping("/actions")
    public ResponseEntity<List<Action>> getAllActions(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("commands-is-null".equals(filter)) {
            log.debug("REST request to get all Actions where commands is null");
            return new ResponseEntity<>(actionService.findAllWhereCommandsIsNull(),
                    HttpStatus.OK);
        }
